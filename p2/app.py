...
def say_hello(hello_to):
    with tracer.start_span('app-init') as span:
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://bui2020:BUI-zimowy-2020@172.17.0.9/studia'

app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False


with tracer.start_span('db-init') as span:
    db = SQLAlchemy(app)
...
class Studenci(db.Model):
    ...
    def __init__(self, nr_indeksu, imie, nazwisko):
        with tracer.start_span('studenci-init') as span:
...
class Prowadzacy(db.Model):
    ...
    def __init__(self, nr_prowadzacego, imie, nazwisko):
        with tracer.start_span('prowadzacy-init') as span:
...
class Kursy(db.Model):
    ...
    def __init__(self, nr_kursu, nazwa, nr_gl_prowadzacego):
        with tracer.start_span('kursy-init') as span:
...
@app.route('/')
def Index():
    with tracer.start_span('render_index') as span:
